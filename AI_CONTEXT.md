...
- Исправлена гонка первого autoplay: `video.load()` теперь выполняется только один раз после ленивого добавления источников, а отдельный эффект управляет `play()`/`pause()` без повторного сброса media pipeline при изменении видимости.
- Регрессионный тест отдельно проверяет первый вход в playback-зону, остановку при уходе и повторный запуск при возврате; повторные viewport-переходы не вызывают новый `load()`.
...
- Коммит `de9c5b9` развёрнут в production 28 июля 2026 года как Docker-образ `sk-develop:de9c5b9`; предыдущий контейнер `ca5bc01` сохранён остановленным для отката.
- После production-деплоя в браузере подтверждены первый autoplay при однократном скролле к блоку, остановка вне viewport и повторный запуск при возврате; Console без ошибок.
